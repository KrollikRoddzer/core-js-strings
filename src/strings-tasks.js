...
function sumOfCodes(str) {
  ...
  return str
    .split('')
    .reduce((acc, cur, index) => acc + str.charCodeAt(index), 0);
}
...
function countVowels(str) {
  const arr = ['a', 'y', 'i', 'o', 'e', 'A', 'Y', 'I', 'O', 'E', 'u', 'U'];
  return str
    .split('')
    .reduce((acc, cur) => (arr.includes(cur) ? 1 : 0) + acc, 0);
}
...
function isPalindrome(str) {
  ...
  const str1 = strModified.reverse().join('');
  const str2 = strModified.reverse().join('');
  return str1 === str2;
}
...
function reverseWords(str) {
  let strs = str.split(' ');
  strs = strs.map((item) => item.split('').reverse().join(''));
  return strs.join(' ');
}
...
function extractNameFromTemplate(value) {
  ...
  return splited[splited.length - 2]
    .concat(' ')
    .concat(
      splited[splited.length - 1].substring(
        0,
...
function encodeToRot13(str) {
  ...
  for (let i = 65; i <= 90; i += 1) {
    obj[String.fromCharCode(i)] = String.fromCharCode(
      i + 13 > 90 ? i - 13 : i + 13
    );
  }
